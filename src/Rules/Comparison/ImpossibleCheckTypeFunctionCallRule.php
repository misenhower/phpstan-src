...
final class ImpossibleCheckTypeFunctionCallRule implements Rule
{
	...
	public function processNode(Node $node, Scope&NodeCallbackInvoker&CollectedDataEmitter $scope): array
	{
		...
		$reasons = [];
		$isAlways = $this->impossibleCheckTypeHelper->findSpecifiedType($scope, $funcCall, $nodeResult, $reasons);
		if ($isAlways === null) {
		...
		$addTip = function (RuleErrorBuilder $ruleErrorBuilder) use ($scope, $funcCall, $nodeResult, $reasons): RuleErrorBuilder {
			...
			$isAlways = $this->impossibleCheckTypeHelper->doNotTreatPhpDocTypesAsCertain()->findSpecifiedType($scope, $funcCall, $nodeResult, $reasons);
			if ($isAlways !== null) {
				return $this->possiblyImpureTipHelper->addTip($scope, $funcCall, $ruleErrorBuilder);
			}
