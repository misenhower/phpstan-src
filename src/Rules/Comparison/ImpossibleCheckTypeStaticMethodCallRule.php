...
final class ImpossibleCheckTypeStaticMethodCallRule implements Rule
{
	...
	public function processNode(Node $node, Scope&NodeCallbackInvoker&CollectedDataEmitter $scope): array
	{
		...
		$reasons = [];
		$isAlways = $this->impossibleCheckTypeHelper->findSpecifiedType($scope, $staticCall, $nodeResult, $reasons);
		if ($isAlways === null) {
		...
		$addTip = function (RuleErrorBuilder $ruleErrorBuilder) use ($scope, $staticCall, $nodeResult, $reasons): RuleErrorBuilder {
			...
			$isAlways = $this->impossibleCheckTypeHelper->doNotTreatPhpDocTypesAsCertain()->findSpecifiedType($scope, $staticCall, $nodeResult);
			if ($isAlways !== null) {
				return $this->possiblyImpureTipHelper->addTip($scope, $staticCall, $ruleErrorBuilder);
			}
		...
		if (!$isAlways) {
			$method = $this->getMethod($staticCall->class, $staticCall->name->name, $scope);
